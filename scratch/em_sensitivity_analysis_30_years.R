...
proc_error_v <- c(0.2, 0.5, 0.75, 1) # Process error for NAA random effects
mse_gaps_v <- c(3,6) # Time between assessments for MSE
gauss_width_v <- c(0.25, 0.5, 1) # Width of the gaussian relationship (Wider = Less sensitive to optimal temperature)
...
if(DO_ANALYSIS){
  ...
  # IMPORATANT #
  # don't use "i", i would be overwrited. #
  # IMPORATANT #
  ...
  for(j in 1:nrow(sens_analysis_settings)) {
    ...
    tryCatch({
      ...
      if(SAVE_MODEL){
        ...
        #                            mod4_em_pred_catch)
        mod_em_pred_catch <- rbind(mod1_em_pred_catch,
                                   mod2_em_pred_catch, mod3_em_pred_catch)
        ...
        mod_NAA <- rbind(mod1_em_NAA, mod2_em_NAA, mod3_em_NAA)
        ...
        mod_NAA_plot_1 <- ggplot(mod_NAA_df, aes(year, NAA_1, color=as.factor(model))) +
          ...
          assess_years_lines +
          scale_x_continuous(breaks=seq(1985,2040,5)) +
          labs(color="Model", title=paste("NAA 1 in EM: ","sigma_naa=",proc_error, ", mse_gap=",mse_gap, ", gaussian_width=",gauss_width, sep=" ")) +
          theme_bw() +
          theme(axis.text.x=element_text(angle=60, vjust=1, hjust=1))
        ...
        ##### FLEET 1 (COMMERCIAL) - FISHING PRESSURE #####
        ...
        mod1_em_fbar_f1 <- data.frame(fbar=mod1$em_full[[1]]$rep$Fbar[,1])
        mod1_em_fbar_f1 <- cbind(mod1_em_fbar_f1,model='Model 1 - EM')
        mod1_em_fbar_f1 <- cbind(ID = as.integer(1:nrow(mod1_em_fbar_f1)), mod1_em_fbar_f1)
        mod2_em_fbar_f1 <- data.frame(fbar=mod2$em_full[[1]]$rep$Fbar[,1])
        mod2_em_fbar_f1 <- cbind(mod2_em_fbar_f1,model='Model 2 - EM')
        mod2_em_fbar_f1 <- cbind(ID = as.integer(1:nrow(mod2_em_fbar_f1)), mod2_em_fbar_f1)
        mod3_em_fbar_f1 <- data.frame(fbar=mod3$em_full[[1]]$rep$Fbar[,1])
        mod3_em_fbar_f1 <- cbind(mod3_em_fbar_f1,model='Model 3 - EM')
        mod3_em_fbar_f1 <- cbind(ID = as.integer(1:nrow(mod3_em_fbar_f1)), mod3_em_fbar_f1)
        
        # mod_fbar <- rbind(mod1_om_fbar, mod1_em_fbar, mod2_em_fbar, mod3_em_fbar, mod4_em_fbar)
        mod_fbar_f1 <- rbind(mod1_em_fbar_f1, mod2_em_fbar_f1, mod3_em_fbar_f1)
        mod_fbar_df_f1 <- tibble::as_tibble(mod_fbar_f1)
        mod_fbar_df_f1 <- mod_fbar_df_f1 %>% rename(year=ID) %>% mutate(year=as.integer(year)+1988)
        mod_fbar_df_f1 <- mod_fbar_df_f1 %>% mutate(fleet="Fleet 1", .before=3)

        ##### FLEET 2 (RECREATION) - FISHING PRESSURE #####
        # mod1_om_fbar <- data.frame(fbar=mod1$om$rep$Fbar[,1])
        # mod1_om_fbar <- cbind(mod1_om_fbar,model='OM')
        # mod1_om_fbar <- cbind(ID = as.integer(1:nrow(mod1_om_fbar)), mod1_om_fbar)
        mod1_em_fbar_f2 <- data.frame(fbar=mod1$em_full[[1]]$rep$Fbar[,2])
        mod1_em_fbar_f2 <- cbind(mod1_em_fbar_f2,model='Model 1 - EM')
        mod1_em_fbar_f2 <- cbind(ID = as.integer(1:nrow(mod1_em_fbar_f2)), mod1_em_fbar_f2)
        mod2_em_fbar_f2 <- data.frame(fbar=mod2$em_full[[1]]$rep$Fbar[,2])
        mod2_em_fbar_f2 <- cbind(mod2_em_fbar_f2,model='Model 2 - EM')
        mod2_em_fbar_f2 <- cbind(ID = as.integer(1:nrow(mod2_em_fbar_f2)), mod2_em_fbar_f2)
        mod3_em_fbar_f2 <- data.frame(fbar=mod3$em_full[[1]]$rep$Fbar[,2])
        mod3_em_fbar_f2 <- cbind(mod3_em_fbar_f2,model='Model 3 - EM')
        mod3_em_fbar_f2 <- cbind(ID = as.integer(1:nrow(mod3_em_fbar_f2)), mod3_em_fbar_f2)
        
        # mod_fbar <- rbind(mod1_om_fbar, mod1_em_fbar, mod2_em_fbar, mod3_em_fbar, mod4_em_fbar)
        mod_fbar_f2 <- rbind(mod1_em_fbar_f2, mod2_em_fbar_f2, mod3_em_fbar_f2)
        mod_fbar_df_f2 <- tibble::as_tibble(mod_fbar_f2)
        mod_fbar_df_f2 <- mod_fbar_df_f2 %>% rename(year=ID) %>% mutate(year=as.integer(year)+1988)
        mod_fbar_df_f2 <- mod_fbar_df_f2 %>% mutate(fleet="Fleet 2", .before=3)
        
        ##### MEAN FISHING PRESSURE (F1+F2) #####
        # mod1_om_fbar <- data.frame(fbar=mod1$om$rep$Fbar[,1])
        # mod1_om_fbar <- cbind(mod1_om_fbar,model='OM')
        # mod1_om_fbar <- cbind(ID = as.integer(1:nrow(mod1_om_fbar)), mod1_om_fbar)
        mod1_em_fbar_f3 <- data.frame(fbar=mod1$em_full[[1]]$rep$Fbar[,3])
        mod1_em_fbar_f3 <- cbind(mod1_em_fbar_f3,model='Model 1 - EM')
        mod1_em_fbar_f3 <- cbind(ID = as.integer(1:nrow(mod1_em_fbar_f3)), mod1_em_fbar_f3)
        mod2_em_fbar_f3 <- data.frame(fbar=mod2$em_full[[1]]$rep$Fbar[,3])
        mod2_em_fbar_f3 <- cbind(mod2_em_fbar_f3,model='Model 2 - EM')
        mod2_em_fbar_f3 <- cbind(ID = as.integer(1:nrow(mod2_em_fbar_f3)), mod2_em_fbar_f3)
        mod3_em_fbar_f3 <- data.frame(fbar=mod3$em_full[[1]]$rep$Fbar[,3])
        mod3_em_fbar_f3 <- cbind(mod3_em_fbar_f3,model='Model 3 - EM')
        mod3_em_fbar_f3 <- cbind(ID = as.integer(1:nrow(mod3_em_fbar_f3)), mod3_em_fbar_f3)
        
        # mod_fbar <- rbind(mod1_om_fbar, mod1_em_fbar, mod2_em_fbar, mod3_em_fbar, mod4_em_fbar)
        mod_fbar_f3 <- rbind(mod1_em_fbar_f3, mod2_em_fbar_f3, mod3_em_fbar_f3)
        mod_fbar_df_f3 <- tibble::as_tibble(mod_fbar_f3)
        mod_fbar_df_f3 <- mod_fbar_df_f3 %>% rename(year=ID) %>% mutate(year=as.integer(year)+1988)
        mod_fbar_df_f3 <- mod_fbar_df_f3 %>% mutate(fleet="Combined F", .before=3)
        
        # COMBINE ALL THREE DATAFRAMES
        mod_fbar_df <- rbind(mod_fbar_df_f1, mod_fbar_df_f2, mod_fbar_df_f3)
        
        ##### PLOT THE FISHING PRESSURES #####
        ###### SEPARATELY ######
        # Plot via ggplot
        ###### PLOT FLEET 1 F ######
        mod_fbar_f1_plot_1 <- ggplot(mod_fbar_df_f1, aes(year, fbar, color=as.factor(model))) +
          geom_line(alpha=1.0, linewidth=1) + facet_wrap(~model, nrow=2) +
          assess_years_lines +
          scale_x_continuous(breaks=seq(1985,2040,5)) +
          labs(color="Model", title=paste("Fbar for Fleet 1 in EM: ","sigma_naa=",proc_error, ", mse_gap=",mse_gap, ", gaussian_width=",gauss_width, sep=" ")) +
          theme_bw() +
          theme(axis.text.x=element_text(angle=60, vjust=1, hjust=1))

        mod_fbar_f1_plot_2 <- ggplot(mod_fbar_df_f1, aes(year, fbar, color=as.factor(model))) +
          geom_line(alpha=1.0, linewidth=1) +
          assess_years_lines +
          scale_x_continuous(breaks=seq(1985,2040,5)) +
          labs(color="Model") +
        ...
        ###### PLOT FLEET 2 F ######
        mod_fbar_f2_plot_1 <- ggplot(mod_fbar_df_f2, aes(year, fbar, color=as.factor(model))) +
          geom_line(alpha=1.0, linewidth=1) + facet_wrap(~model, nrow=2) +
          assess_years_lines +
          scale_x_continuous(breaks=seq(1985,2040,5)) +
          labs(color="Model", title=paste("Fbar for Fleet 2 in EM: ","sigma_naa=",proc_error, ", mse_gap=",mse_gap, ", gaussian_width=",gauss_width, sep=" ")) +
          theme_bw() +
          theme(axis.text.x=element_text(angle=60, vjust=1, hjust=1))
        
        # Plot via ggplot
        mod_fbar_f2_plot_2 <- ggplot(mod_fbar_df_f2, aes(year, fbar, color=as.factor(model))) +
          geom_line(alpha=1.0, linewidth=1) +
          assess_years_lines +
          scale_x_continuous(breaks=seq(1985,2040,5)) +
          labs(color="Model") +
        ...
        ###### PLOT COMBINED FLEET ######
        mod_fbar_f3_plot_1 <- ggplot(mod_fbar_df_f3, aes(year, fbar, color=as.factor(model))) +
          geom_line(alpha=1.0, linewidth=1) + facet_wrap(~model, nrow=2) +
          assess_years_lines +
          scale_x_continuous(breaks=seq(1985,2040,5)) +
          labs(color="Model", title=paste("Fbar for combined fleet in EM: ","sigma_naa=",proc_error, ", mse_gap=",mse_gap, ", gaussian_width=",gauss_width, sep=" ")) +
          theme_bw() +
          theme(axis.text.x=element_text(angle=60, vjust=1, hjust=1))
        
        # Plot via ggplot
        mod_fbar_f3_plot_2 <- ggplot(mod_fbar_df_f3, aes(year, fbar, color=as.factor(model))) +
          geom_line(alpha=1.0, linewidth=1) +
          assess_years_lines +
          scale_x_continuous(breaks=seq(1985,2040,5)) +
          labs(color="Model") +
        ...
        ###### PLOT Ffor all fleets together on same plot ######
        ###### PLOT COMBINED FLEET ######
        mod_fbar_all_plot_1 <- ggplot(mod_fbar_df, aes(year, fbar, linetype=as.factor(fleet),
                                                       color=as.factor(model))) +
          geom_line(alpha=1.0, linewidth=0.5, ) + facet_wrap(~model, nrow=2) +
          assess_years_lines +
          scale_x_continuous(breaks=seq(1985,2040,5)) +
          labs(color="Model", linetype="Fleet", title=paste("Fbar for combined fleet in EM: ","sigma_naa=",proc_error, ", mse_gap=",mse_gap, ", gaussian_width=",gauss_width, sep=" ")) +
          theme_bw() +
          theme(axis.text.x=element_text(angle=60, vjust=1, hjust=1))
        
        # Plot via ggplot
        mod_fbar_all_plot_2 <- ggplot(mod_fbar_df, aes(year, fbar, linetype=as.factor(fleet),
                                                         color=as.factor(model))) +
          geom_line(alpha=1.0, linewidth=0.5) +
          assess_years_lines +
          scale_x_continuous(breaks=seq(1985,2040,5)) +
          labs(color="Model", linetype="Fleet") +
          theme_bw() +
          theme(axis.text.x=element_text(angle=60, vjust=1, hjust=1))
        ...
        rec_par_df <- rbind(rec_par_mod_2_1, rec_par_mod_3_1,
                            rec_par_mod_2_3)
        ...
        diagnostics_plots <- list(om_ssb_plot_1, om_ssb_plot_2,
                                  ...
                                  mod_fbar_f1_plot_1, mod_fbar_f1_plot_2,
                                  mod_fbar_f2_plot_1, mod_fbar_f2_plot_2,
                                  mod_fbar_f3_plot_1, mod_fbar_f3_plot_2,
                                  mod_fbar_all_plot_1, mod_fbar_all_plot_2)
