...
# Here we will try to introduce warming rates to the assessment period at 0.04 deg C/year with some variability
#
# -----------------------------------------------------------------------------
...
temp_trend_v <- c(0.0, 0.04, 0.1) # Trend of the future temperature scenario
temp_trend_stoch_v <- c(0.1, 0.25, 0.5, 0.75) # Stochasticity of the future temperature trend
topt_rec_v <- c(0.0) # The optimal temperature at which recruitment is maximized
total_comb_no <- length(proc_error_v)*length(mse_gaps_v)*length(gauss_width_v)*length(temp_trend_v)*length(temp_trend_stoch_v)*length(topt_rec_v)
...
sens_analysis_settings <- tidyr::crossing(proc_error= proc_error_v,
                                          ...
                                          temp_trend=temp_trend_v,
                                          temp_trend_stoch= temp_trend_stoch_v,
                                          topt_rec=topt_rec_v)
...
# Re-source custom functions on each worker
clusterEvalQ(cl, {
  source(file.path(project_root, "functions", "reusable_functions.R"))
...
if(DO_ANALYSIS){
  for(j in 1:nrow(sens_analysis_settings)){
    j_val <- j
    foreach(k=1:nrow(random_seeds_df),
            .packages = c("wham", "whamMSE", "dplyr", "ggplot2", "readr", "beepr", "here")
    ) %dopar%{
      tryCatch({
        mod1_flag <- FALSE
        mod2_flag <- FALSE
        ...
        row <- sens_analysis_settings[j_val,]
        proc_error <- row$proc_error
        mse_gap <- row$mse_gap
        gauss_width <- row$gauss_width
        topt_rec <- row$topt_rec
        temp_trend <- row$temp_trend
        temp_trend_stoch <- row$temp_trend_stoch
        run_id <- row$nid
        ...
        input_Ecov$par$Topt_rec      <- topt_rec  # NOTE: This has been changed
        ...
        ecov_proj_error <- rnorm(n = n_proj, mean = 0, sd = temp_trend_stoch)
        
        input_Ecov$par$Ecov_re[proj_idx, 1] <- temp_trend * seq_len(n_proj) + last_ecov_temp + ecov_proj_error # Temp trend is now set from the outside
        ...
        mod3 <- loop_through_fn(
          ...
          gauss_rec_em = list( # This will create the linkage
            ...
            Topt_rec = topt_rec, # This is now set from the external settings
      ...
      error=function(e){
        ...
        if(SAVE_MODEL){
          ...
          # # Create a folder. Suppress warnings and allow recursive folders to be created
          # dir.create(folder_path, recursive = TRUE, showWarnings = FALSE)
          saveRDS(mod1, here(folder_path,paste("sens_run_",run_id,"_iter_id_",iter_id,"_error_log_mod_1",".RDS",sep="")))
          saveRDS(mod2, here(folder_path,paste("sens_run_",run_id,"_iter_id_",iter_id,"_error_log_mod_2",".RDS",sep="")))
          saveRDS(mod3, here(folder_path,paste("sens_run_",run_id,"_iter_id_",iter_id,"_error_log_mod_3",".RDS",sep="")))
          print(paste("Models saved for Run",run_id,"and seed: ",seed,sep=" "))
...
print("Parallelized code is done!")


# proc_error_v <- c(0.2) # Process error for NAA random effects. We keep a low value here to see that allows us to see difference in model performance
# mse_gaps_v <- c(6) # Time between assessments for MSE
# gauss_width_v <- c(2) # Width of the gaussian relationship (Wider = Less sensitive to optimal temperature)
# temp_trend_v <- c(0.0) # Trend of the future temperature scenario
# temp_trend_stoch_v <- c(0.5) # Stochasticity of the future temperature trend
# topt_rec_v <- c(0.0) # The optimal temperature at which recruitment is maximized


# Save the diagnostic information and model meta information
