...
### EM-0 (baseline): no Ecov included
...
### EM-1 (true-link benchmark): Gaussian recruitment–temperature link with fixed parameters

EM-1 represented a best-case benchmark in which the EM uses the same Gaussian recruitment–temperature relationship as the OM. The Gaussian parameters \((T_{\text{opt}}, w_{\text{opt}})\) were treated as fixed and known in EM-1 (i.e., not estimated). Using the observed Ecov series \(x_y\) available to the EM, the multiplier was:
...
### EM-2: Ecov AR(1) state-space model with observation error, no recruitment–Ecov relationship

EM-2 included Ecov as a latent AR(1) state-space process with observation error, but assumed no relationship between Ecov and recruitment. Let \(X_y\) denote the latent Ecov state and \(x_y\) the observed Ecov input. The Ecov process was:

\begin{equation}
X_1 \sim \mathcal{N}\left(\mu_{\text{Ecov}},\;\frac{\sigma_{\text{Ecov}}^2}{1-\rho_{\text{Ecov}}^2}\right),
\label{eq:em_ecov_init}
\end{equation}

\begin{equation}
X_y \mid X_{y-1} \sim \mathcal{N}\left(\mu_{\text{Ecov}}(1-\rho_{\text{Ecov}})+\rho_{\text{Ecov}}X_{y-1},\;\sigma_{\text{Ecov}}^2\right),\quad y=2,\ldots,Y,
\label{eq:em_ecov_ar1}
\end{equation}

and the observation model was:

\begin{equation}
x_y \mid X_y \sim \mathcal{N}\left(X_y,\;\sigma_x^2\right).
\label{eq:em_ecov_obs}
\end{equation}

Recruitment in EM-2 was independent of Ecov and followed Eq. \eqref{eq:em0_rec}. In forecast years, EM-2 generated future Ecov states by continuing the fitted AR(1) process forward using Eq. \eqref{eq:em_ecov_ar1}. Although Ecov did not affect recruitment in EM-2, this continuation ensured that projected Ecov dynamics remained internally consistent with the EM’s assumed state-space structure.

### EM-3 (Linear relationship between recruitment and temperature)

EM-3 uses an existing option in whamMSE where a linear relationship between recrutiment-temperature is estimated. Recruitment is defined as: 
...
2.  \(t_{\text{g}} = \{3\ \text{years}, 6\ \text{years}\}\)
3.  \(w_{\text{opt}} = \{0.5, 1, 2k\}\) where 0.1 is an extremely narrow temperature response curve and 5 is very generous and will give good recruitment responses even to temperatures far from optimal value.
